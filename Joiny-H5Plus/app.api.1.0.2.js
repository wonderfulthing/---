(function(){
    ...
    function requestPayment(params){
        ...
        plus.payment.request( channel, params.statement , function(res){
    ...
    function readTxtFile(params){
        ...
        plus.io.resolveLocalFileSystemURL( txtFilePath , function( entry ) {
    ...
    function uploadFile(params){
        ...
        var task = plus.uploader.createUpload( params.url , options ,
            function ( t, status ) {
                // 上传完成
                if ( status == 200 ) {
                    try{
...
// 创建Barcode扫码控件
    ...
    function createBarcode(params) {
        ...
        if(!barcode){
            barcode = plus.barcode.create('barcode', [plus.barcode.QR], defaultStyle );
    ...
    function galleryImg(params) {
        ...
        plus.gallery.pick( function(res){
            ...
            plus.nativeUI.alert('最多只能选择'+ params.count +'张图片');
        }});
    }
    ...
    function savePicture(params) {
        ...
        if(filePath.indexOf('data:image/') == 0){
            ...
            loadBase64(filePath , params , function(localFilePath){
                plus.gallery.save( localFilePath , function (e) {
